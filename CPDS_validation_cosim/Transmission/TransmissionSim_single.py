...
load_timeseries = consistent_random_object.normal(1, 0.0, int(total_time)) # used to be 0.02
...
for request_time in np.arange(0, total_time, simulation_step_time): # current time is end of simulation of andes
    ...
    #     print(f"PVD_AGC_response_np.shape={PVD_AGC_response_np.shape}")
    #     if DER_AGC_flag:
...
dae_DER_output = ss.dae.ts.get_data([ss.PVD1.Ipout_y],a=[0,])*ss.dae.ts.get_data([ss.PVD1.v],a=[0,])*100
...
freq_ACE_df = pd.DataFrame(freq_ACE_np, columns=['time_s','frequency_Hz','ACE_MW', 'Pext_AGC', 'TGOV1N_AGC', 'DER_output'])
